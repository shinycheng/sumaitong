...
for i in range(lows.max_row+1):
    ...
    js = lows.cell(row=i+1,column=1).value
    aparams.append([js,(
        ('dateType', 'recent30'),
...
class Mythread(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            try:
                all_dicts(self.name,self.p)
            except Exception as e:
                print(e)
                break
def all_dicts(name,p):
    pa = p.get(timeout = 2)
    params = pa[1]
    keys = []
    values = []
    ...
    ali_dict['商品浏览率'] = str((data['vstItemPercent']['value']) * 100) + '%'
    for key in ali_dict.keys():
        keys.append(key)
    for i in range(len(keys)):
        ws.cell(row=1, column=i + 1).value = keys[i]
    for value in ali_dict.values():
        values.append(value)
    ws.append(values)
